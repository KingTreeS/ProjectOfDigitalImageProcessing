...
//-------------------------------------------------------------------------------
bool JpegEncoder::lightReverse(const char *fileName, int quality_scale)
{
    //尚未读取？
    if(m_rgbBuffer==0 || m_width==0 || m_height==0) return false;
    ...
    for(int yPos=0; yPos<m_height; yPos+=8)
    {
        for (int xPos=0; xPos<m_width; xPos+=8)
        {
            ...
            //对颜色进行取反
            _inverseColorSpace(xPos, yPos, yData, cbData, crData);
...
//-------------------------------------------------------------------------------
void JpegEncoder::_inverseColorSpace(int xPos, int yPos, char* yData, char* cbData, char* crData)
{
	for (int y=0; y<8; y++)
	{
		...
		for (int x=0; x<8; x++)
		{
			...
			yData[y*8+x] = (char)(255 - (0.299f * R + 0.587f * G + 0.114f * B - 128));
			cbData[y*8+x] = (char)(255 - (-0.1687f * R - 0.3313f * G + 0.5f * B ));
			crData[y*8+x] = (char)(255 - (0.5f * R - 0.4187f * G - 0.0813f * B));
		}
	}
}
